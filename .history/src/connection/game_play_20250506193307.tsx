...
export function submitAnswer(user_auth: string, password: string){

    const phaseIndex = users[user_auth].currentGame[1] || 0;
    const gameId = users[user_auth].currentGame;
    
    if (gameId && (gameId in games)) {
        const currentPhaseData = games[gameId].phases[phaseIndex];
        const correctPassword = currentPhaseData.password;
        if (password.toLowerCase() === correctPassword.toLowerCase()) {
            // Password is correct
            setCurrentPhase(prev => prev + 1);
            if (currentPhase === game.length - 1) {
                // Game completed
        ...
        const { password_, ...rest } = phases[phaseIndex];
        return {
            phase: rest,
            length: length,
            title: title,
